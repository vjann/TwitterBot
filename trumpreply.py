...
lasttweet = api.user_timeline('realDonaldTrump')[0]
...
def runTime():
    ...
    mostrecenttweet = api.user_timeline('realDonaldTrump')[0]
    ...
    if mostrecenttweet != lasttweet:
        if lasttweet.id%6 == 0:
            api.update_with_media('meme.png',"@realDonaldTrump", in_reply_to_status_id = mostrecenttweet.id)
            print('tweeting')
            #api.update_status("@vjannitor nope", mostrecenttweet.id)
        elif lasttweet.id%6 ==1:
            api.update_status("@realDonaldTrump " + f[0], mostrecenttweet.id)
            print('tweeting')
        elif lasttweet.id%6 ==2:
            api.update_status("@realDonaldTrump " + f[1], mostrecenttweet.id)
            print('tweeting')
        elif lasttweet.id%6 ==3:
            api.update_status("@realDonaldTrump " + f[2], mostrecenttweet.id)
            print('tweeting')
        elif lasttweet.id%6 ==4:
            api.update_status("@realDonaldTrump " + f[3], mostrecenttweet.id)
            print('tweeting')
        elif lasttweet.id%6 ==5:
            api.update_status("@realDonaldTrump " + f[4], mostrecenttweet.id)
            print('tweeting')
